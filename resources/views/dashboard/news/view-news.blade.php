    ...
    <div class="main_container  main_menu_open">
        ...
        <div class="home_pass hidden-xs">
            <ul>
                ...
                <li class="bring_right"><a href="">عرض  الاخبار</a></li>
            </ul>
        </div>
        <!--/End system bath-->
        <div class="page_content">
            <h1 class="heading_title">عرض كل  الاخبار </h1>
            @include('dashboard.layouts.flash-message')
            <button type="button" class="btn btn-primary" class="add-project-sidebar" href="javascript:void(0)"  data-toggle="modal" data-target="#addProjectSidebar" >إضافة خبر جديد</button>
            {{-- add form --}}
            <div class="modal fade" id="addProjectSidebar">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">إضافة خبر جديد</h5>
            ...
            <div class="wrap">
                <table id="example" class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tr>
                        <td>{{$key+1}}</td>
                        <td><img src="{{(!empty($news->image))?url('upload/image/'.$news->image):url('upload/no_image.jfif')}}" width="120px" height="130px" alt="image"></td>
                        <td>{{$news->title_ar}}</td>
                        <td>{{$news->title_en}}</td>
                        <td>{!! $news->description_ar !!}</td>
                        <td>{!! $news->description_en !!}</td>
                        <td>
                            <a href="{{route('news.edit',$news->id)}}" class="glyphicon glyphicon-pencil" data-toggle="tooltip" data-placement="top" title="تعديل"></a>
                            <a href="{{route('news.delete',$news->id)}}" class="glyphicon glyphicon-remove" data-toggle="tooltip" data-placement="top" title="حذف"></a>
                        </td>
                    </tr>
                @endforeach
                </table>
